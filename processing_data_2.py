    ...
    for i in range(5):
        ...
        for _ in range(_counts[i]):
            try:
                ...
                tmp_image_urls = str(df_dict['image_urls']).split("***https://cdn.nhathuoclongchau.com.vn/unsafe/150x0/filters:quality(90)/")
                image_urls = ""
                for image_url in tmp_image_urls:
                    if len(image_urls) > 0:
                        image_urls += "***"
        ...
        new_df.to_csv(f"./final_result_2/{_categories[i]}.csv", index=False)
